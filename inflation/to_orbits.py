...
class packed_inflated_columns(inflated_hypergraph, DAG):
    ...
    def __init__(self, hypergraph, inflation_orders, directed_structure, outcome_cardinalities,
        ...
        self.inflated_packed_cardinalities_indecies = np.repeat(np.arange(len(outcome_cardinalities)),
...
class expressible_sets(packed_inflated_columns):
    ...
    def __init__(self, hypergraph, inflation_orders, directed_structure, outcome_cardinalities,
                 private_setting_cardinalities):
        packed_inflated_columns.__init__(self, hypergraph, inflation_orders, directed_structure, outcome_cardinalities,
                                         private_setting_cardinalities)
        self.unpacked_conf_integers = np.array([list(
            np.arange(self.setting_cardinalities[packed_obs]) + np.array(self.setting_cardinalities)[
                self.inflated_packed_cardinalities_indecies[:packed_obs_index]].sum()) for packed_obs_index, packed_obs
                                                in enumerate(self.inflated_packed_cardinalities_indecies)],
                                               dtype=object)
        ...
        self.conf_setting_indecies = self.conf_setting_integers[
        ...
        self.ravelled_conf_var_indecies = np.repeat(np.arange(self.observed_count),
    ...
    def eset_discarded_rows_to_trash(self,partitioned_eset,offset):
        ...
        which_rows_to_keep = np.intersect1d(self.eset_unpacking_rows_to_keep(partitioned_eset),
                                            self.eset_symmetry_rows_to_keep(partitioned_eset, shape_of_eset,
                                                                            size_of_eset))
    ...
    @staticmethod
    def columns_to_unique_rows(shaped_column_integers, flat_eset):
        ...
        reshaped_column_integers = shaped_column_integers.transpose(MoveToBack(len(data_shape), flat_eset)).reshape(
            (-1, expr_set_size))
...
class inflation_problem(expressible_sets):
    ...
    def __init__(self, hypergraph, inflation_orders, directed_structure, outcome_cardinalities,
                 private_setting_cardinalities):
        expressible_sets.__init__(self, hypergraph, inflation_orders, directed_structure, outcome_cardinalities,
                                  private_setting_cardinalities)
